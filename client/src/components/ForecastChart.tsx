...
interface ForecastChartProps {
  data: Array<{ year: number; price: number; lowerBound: number; upperBound: number }>;
}
    ...
    <Card className="shadow-soft border-border/50 relative overflow-hidden">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg font-semibold text-slate-800">Future Projections (Next 10Y)</CardTitle>
            <CardDescription>AI-model forecast with confidence range</CardDescription>
          </div>
      ...
      <CardContent>
            ...
            <ComposedChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                formatter={(value: number, name: string) => {
                  ...
                  if (name === "range") return [null, 'Range']; // Hide from tooltip
                  return [value, name];
                }}
              ...
              <Line 
                ...
                strokeWidth={3}
                strokeDasharray="5 5"
                dot={{ r: 4, fill: "hsl(172 66% 50%)", strokeWidth: 0 }}
                activeDot={{ r: 6 }}
              />
            </ComposedChart>
          </ResponsiveContainer>
        ...
        <p className="mt-4 text-xs text-slate-400 text-center">
          *Projections are estimates based on historical growth patterns and market sentiment.
        </p>
      </CardContent>
    </Card>
  );
